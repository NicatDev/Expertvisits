...
import { useRef, useLayoutEffect, memo, forwardRef } from 'react';
import { toast } from 'react-toastify';
import { useTranslation } from '@/i18n/client';
import { applyArticleLinkPolicyToDom } from '@/lib/utils/articleLinkPolicy';
...
/**
 * `dangerouslySetInnerHTML` olan düyünü valideyn renderlarında təkrar yazmamaq üçün memo.
 * Əks halda React `__html` eyni olsa belə bəzən daxili DOM-u sıfırlayır və kod toolbar itir.
 */
const MemoArticleHtml = memo(
    forwardRef(function MemoArticleHtml({ html, className }, ref) {
...
export default function ArticleBodyContent({ html, ariaLabel }) {
    ...
    useLayoutEffect(() => {
        ...
        applyArticleLinkPolicyToDom(root, window.location.origin);
        root.querySelectorAll('a[href]').forEach((a) => {
            a.setAttribute('data-ev-link-policy', '1');
        });
    ...
    return (
        <section className={styles.bodyOuter} aria-label={ariaLabel}>
            <MemoArticleHtml ref={rootRef} html={html} className={styles.body} />
        </section>
    );
}
